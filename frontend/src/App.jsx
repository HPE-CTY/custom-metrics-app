...
function App() {
  ...
  const submitCustomValues = async () => {
    try {
      const response = await axios.post('http://localhost:3000/update-hpa', { averageValue: parseInt(averageValue), averageUtilization: parseInt(averageUtilization)});
  ...
  return (
    <div className="App flex flex-col justify-center items-center gap-4 space-y-4">
      ...
      <input 
        ...
        className='border border-gray-300 p-2 rounded-md outline-none'
      />
      <button onClick={submitCustomValues} className='bg-[#01a982] font-bold'>Enter</button>
